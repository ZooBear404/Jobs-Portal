...
<div class='grid grid-cols-2 gap-2 p-3 rounded-md inside-loop'>
<?php
$job_id = $_GET['id'];
...
foreach ($rows as $job_application) {
	...
	if ($is_reviewed == 'T' && $is_accepted == 'T') {
		echo "<div class='p-3 bg-green-400 rounded-md reviewed_accepted'>";
	} else if ($is_reviewed == 'T') {
		echo "<div class='p-3 bg-orange-300 rounded-md reviewed'>";
			echo "<a href='accept.php?job_application_id=$job_application_id&job_id=$job_id' class='flex justify-center gap-2 p-2 bg-gray-200 rounded-md hover:bg-gray-300 active:bg-gray-400 '><button>Accept</button></a>";
	} else {
		echo "<div class='flex flex-col gap-2 p-3 bg-blue-200 rounded-md not_reviewed'>";
			echo "<div class='grid grid-cols-2 gap-2'>";
				echo "<a href='review.php?job_application_id=$job_application_id&job_id=$job_id' class='p-2 bg-gray-200 rounded-md hover:bg-gray-300 active:bg-gray-400 '><button>Mark Reviewed</button></a>";
				echo "<a href='accept.php?job_application_id=$job_application_id&job_id=$job_id' class='p-2 bg-gray-200 rounded-md hover:bg-gray-300 active:bg-gray-400 '><button>Accept</button></a>";
			echo "</div>";
	}
