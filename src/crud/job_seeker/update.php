...
$info = JobSeekerDatabase\getJobSeekerInfo($job_seeker_id);
				...
				<form action="" method="post" class="h-full">
					...
					<div class="flex flex-col justify-center h-full gap-5">
						<div class="flex flex-col gap-2">
							<div class="flex items-center gap-5 div-first_name">
								...
								<?php
								echo "<input type='text' name='first_name' id='first_name_input' class='p-2 bg-gray-300 rounded-md' value='$first_name' required>";
								?>
							</div>
							<div class="div-last_name">
								<label for="last_name_input">Last Name</label>
								<input type="text" name="last_name" id="last_name_input" class="p-2 bg-gray-300 rounded-md" <?php echo "value='$last_name'"; ?> required>
							</div>
							<div class="div-email">
								<label for="email_input">Email</label>
								<input type="email" name="email" id="email_input" class="p-2 bg-gray-300 rounded-md" <?php echo "value='$email'"; ?> required>
							</div>
							<div class="div-password">
								<label for="password_input">Password</label>
								<input type="password" name="password" id="password_input"  class="p-2 bg-gray-300 rounded-md" <?php echo "value='$password'"; ?> required>
							</div>
							<div class="div-birth">
								<label for="date_of_birth_input">Date of Birth</label>
								<input type="date" name="birth_year" class="p-2 bg-gray-300 rounded-md" <?php echo "value='$date_of_birth'"; ?>>
							</div>
							<div class="gender">
								<label for="gender">Gender</label>
								<input type="radio" name="gender" value="M" id="M-input" class="p-2 bg-gray-300 rounded-md" <?php if ($gender == 'M') echo "checked='checked'"; ?>>
								<label for="M-Input">Male</label>
								<input type="radio" name="gender" value="F" id="F-input" class="p-2 bg-gray-300 rounded-md" <?php if ($gender == 'F') echo "checked='checked'"; ?>>
					...
					<?php
						...
						if ($_SERVER['REQUEST_METHOD'] == 'POST') {
							...
							$result = JobSeekerDatabase\updateJobSeeker($job_seeker_id, $first_name, $last_name, $email, $gender, $date_of_birth, $password);
							if ($result) {
								echo "<p class='success-text'>Successfully updated</p>";
							} else {
